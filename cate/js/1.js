/*! js weidian 2015-06-26 */
function getUserIdFromCookie() {
    return encodeURIComponent(M.getCookie("WD_b_id") || M.getCookie("WD_s_id")) || "-1"
}
var $ = function(a) {
    ...
    B = function(a, b) {
        this.parent = function() {
                ...
                if (b) {
                    ...
                    if (d) {
                        ...
                        if (e = f.classList ? c ?
                                ...
                                function(a) {
                                    var c = a.className; - 1 != c.indexOf(b) && (a.className = c.replace(b, ""))
                                }: function(a) {
                                var c = a.className; - 1 == c.indexOf(b) && (a.className = c + " " + b)
                            },
                            1 === d) return e(a.length ? a[0] : a),
                            this;
                        for (var g = d; g--;) e(a[g]);
                        return a[d - 1]
                    }
            ...
            this.getDom = function() {
                return "[object NodeList]" !== Object.prototype.toString.call(a) ? [a] : a
            }
    },
    M = {
        ...
        setSessionStorage: function(a, b) {
            window.localStorage ? (console.log("浏览支持localStorage"), sessionStorage.setItem(a, b)) : console.log("浏览暂不支持localStorage")
        },
        ...
        switchCookie: function() {
            var a = M.getCookie("WD_clear_domain"),
                b = M.getCookie("WD_a"),
                c = M.getCookie("WD_close_favor"),
                d = M.getCookie("WD_visit_log"),
                e = M.getCookie("lastReqTime"),
                f = M.getCookie("WD_guid"),
                g = M.getCookie("buyer_id"),
                h = M.getCookie("WD_s_id"),
                i = M.getCookie("WD_s_client"),
                j = M.getCookie("WD_s_wduss"),
                k = M.getCookie("WD_b_id"),
                l = M.getCookie("WD_b_wduss"),
                m = M.getCookie("WD_b_kduss"),
                n = M.getCookie("WD_b_tele"),
                o = M.getCookie("WD_b_country"),
                p = M.getCookie("unreadMsg"),
                q = M.getCookie("WD_wfr"),
                r = M.getCookie("from_appid"),
                s = M.getCookie("apiv"),
                t = M.getCookie("WD_pay"),
                u = M.getCookie("WD_NowPay"),
                v = M.getCookie("WD_unpay_oID"),
                w = M.getCookie("WD_unpay_oString"),
                x = M.getCookie("WD_dlBuyerAppBanner"),
                y = M.getCookie("IM_enter"),
                z = M.getCookie("IM_login_token"),
                A = M.getCookie("IM_anonymous_token"),
                B = M.getCookie("IM_unRead_fromID"),
                C = M.getCookie("IM_entry"),
                D = M.getCookie("IM_basePortUrl"),
                E = M.getCookie("IM_from_user_id"),
                F = M.getCookie("WD_fIdArray"),
                G = M.getCookie("cart_322121618_totalPrice"),
                H = M.getCookie("userLastReadActive"),
                F = F,
                I = M.getCookie("cart_seller_list"),
                J = new Array,
                K = M.getCookie("cart_inner_item_count");
            if (I) for (var L = I.split(","), N = L.length, O = 0; N > O; O++) J.push(M.getCookie("cart_" + L[O] + "_i"));
            if (!b) var P = M.getCookie("WD_temp_nam"),
                Q = M.getCookie("WD_temp_tele"),
                R = M.getCookie("WD_temp_province"),
                S = M.getCookie("WD_temp_city"),
                T = M.getCookie("WD_temp_district"),
                U = M.getCookie("WD_temp_add"),
                V = M.getCookie("WD_temp_remark"),
                W = M.getCookie("WD_temp_wxID");
            var X = M.getCookie("Market_go_market");
            if (X) var Y = M.getCookie("Market_userid"),
                Z = M.getCookie("Market_client"),
                $ = M.getCookie("Market_wduss"),
                _ = M.getCookie("Market_viewedPage"),
                ab = M.getCookie("Market_viewedPX"),
                bb = M.getCookie("Market_items_viewedPage"),
                cb = M.getCookie("Market_items_viewedPX");
            if (M.clearCookie(), M.setCookie({
                    WD_clear_domain: a,
                    buyer_id: g,
                    WD_guid: f,
                    WD_close_favor: c,
                    ...
                    WD_unpay_oID: v,
                    WD_unpay_oString: w,
                    WD_dlBuyerAppBanner: x,
                    IM_login_token: z,
                    IM_anonymous_token: A,
                    IM_unRead_fromID: B,
                    IM_entry: C,
                    IM_basePortUrl: D,
                    IM_from_user_id: E,
                    cart_seller_list: I,
                    cart_inner_item_count: K,
                    WD_fIdArray: F,
                    cart_322121618_totalPrice: G,
                    userLastReadActive: H,
                    apiv: s
                }), I) for (var L = I.split(","), N = L.length, O = 0; N > O; O++) M.setCookie("cart_" + L[O] + "_i", J[O]);
            ...
            X && M.setCookie({
                ...
                Market_viewedPage: _,
                Market_viewedPX: ab,
                Market_items_viewedPage: bb,
                Market_items_viewedPX: cb
            })
        },
        ua: function() {
        ...
        gaq: function(a) {
            var b = a.split(";"),
                c = b[0],
                d = "";
            d = 1 == b.length ? M.platform() : b[1],
                _paq.push(["trackEvent", c, "click", d])
        },
        ...
        getOpenId: function() {
            var a = {
                code: M.urlQuery("code"),
                state: M.urlQuery("state"),
                guid: M.getCookie("WD_guid"),
                userID: M.getCookie("WD_b_id")
            };
            M.jsonp("http://weidian.com/weixin/service/getOpenid?param=" + M.toJSON(a),
                function() {})
        },
        getItemUrl: function(a, b, c, d) {
            switch (d = d || !0, M.getCookie("sku_appid")) {
                case "com.geili.koudai":
                case "cn.geili.IShopping2":
                    return "ishopping2://?type=0&id=" + b;
                case "com.koudai.weidian.buyer":
                    return "weidianbuyer://?type=0&id=vdian" + b;
                default:
                    return d ? M.sUrl(a, b, c) : M.sUrl(a, b)
            }
        },
        renderHeaderByApp: function() {
            switch (M.getCookie("sku_appid")) {
                case "cn.geili.IShopping2":
                case "com.geili.koudai":
                case "com.koudai.weidian.buyer":
                    $("#common_hd").hide();
                    break;
                case "com.koudai.haidai":
            }
        },
        getSellerAllUrl: function(a) {
            switch (M.getCookie("sku_appid")) {
                case "com.koudai.weidian.buyer":
                    return "weidianbuyer://?type=9&id=" + a;
                case "com.geili.koudai":
                case "cn.geili.IShopping2":
                    return "ishopping2://?type=9&id=shopId_" + a;
                default:
                    return "/?userid=" + a
            }
        },
        urlReferrer: function() {
            sessionStorage.getItem("weidianReferrer") && (window.weidianReferrer = sessionStorage.getItem("weidianReferrer")),
                sessionStorage.setItem("weidianReferrer", document.URL)
        },
        init: function() {
            ...
            M.urlReferrer(),
                console.log("%c手机开店用微店", "color:red"),
                console.log("http://weidian.com"),
                console.log("%c欢迎各路技术大牛携各门派语言入驻微店", "color:blue"),
                console.log("%c简历发送至 hr@koudai.com [注明-来自console]", "color:blue"),
                console.log("%c就！！等！！你！！来！！", "color:red"),
            M.urlQuery("debug") || (console = new Object, console.log = function() {
                return ""
            });
            ...
            } else - 1 != c.indexOf("weidian.com") && (document.domain = "weidian.com");
            if (M.getCookie("WD_guid") || M.setCookie("WD_guid", (new Date).getTime() + "_" + Math.random().toString().substr(2)), M.urlQuery("appid") && M.setCookie("from_appid", M.urlQuery("appid")), M.urlQuery("is_h5") && 1 == M.urlQuery("is_h5") ? M.setCookie("sku_appid", "h5.geili.glmi") : M.urlQuery("appid") && M.setCookie("sku_appid", M.urlQuery("appid"), !0), M.getCookie("WD_i") && M.getCookie("WD_seller")) {
                console.log("base_H5 exchange");
                var f = M.getCookie("WD_seller"),
                    g = M.getCookie("WD_i"),
                    h = g.split("|"),
                    i = h.length,
                    j = "cart_" + f + "_i",
                ...
                for (var l = 0; i > l; l++) Number(M.getCookie("WD_" + h[l])) && k.push(h[l] + "_" + M.getCookie("WD_" + h[l]));
                M.setCookie(j, k.join(",")),
                    M.delCookie("WD_i")
            }
            if (M.urlQuery("b_id") && M.urlQuery("b_uss")) {
                M.switchCookie();
                var m = M.urlQuery("b_id"),
                    n = M.urlQuery("b_uss");
                ...
                M.jsonp("http://login.koudai.com/weidian/convertBuyerIdentity?param=" + M.toJSON(o),
                    function(a) {
                        if (0 === Number(a.status.status_code)) {
                            ...
                            M.setCookie({
                                ...
                                WD_b_wduss: b.wduss,
                                WD_b_kduss: b.kduss,
                                WD_b_tele: b.telephone,
                                WD_b_country: b.country_code
                            })
                        }
                    })
            }
            if (localStorage.length > 1) {
                for (var l in localStorage) M.setCookie(l, localStorage[l]);
                localStorage.clear()
            }
            $(".for_gaq").bind("click",
                function() {
            ...
            (0 === location.href.indexOf("http://wd.koudai.com") || 0 === location.href.indexOf("http://weidian.com")) && (M.getCookie("WD_pay") || M.getCookie("WD_NowPay")) && M.getCookie("WD_unpay_oID") && M.getCookie("WD_unpay_oString") && M.loadScript("http://s.koudai.com/script/common/unpay_top_notice.js"),
            $("#common_hd_logo").length() && $("#common_hd_logo").getDom() && $("#common_hd_logo").getDom()[0].removeAttribute("href"),
            M.urlQuery("code") && M.urlQuery("state") && M.getOpenId(),
                M.urlQuery("fromUser") && "mpwx" == M.urlQuery("fromUser") ? (sessionStorage.mpwxEnter = location.href, sessionStorage.mpwxEnter.length >= 0 && $("#hd_edit").length() > 0 && $("#hd_edit").hide()) : sessionStorage.mpwxEnter && sessionStorage.mpwxEnter.length > 0 && $("#hd_edit").length() > 0 && $("#hd_edit").hide()
        }
    };
M.init();
var _paq = _paq || [];
_paq.push(["setCustomVariable", 1, "appname", "vdian.com", "visit"]),
    _paq.push(["trackPageView"]),
    _paq.push(["enableLinkTracking"]);
var analyHost = "://analysis.koudai.com/",
    is_https = "https:" === document.location.protocol ? 1 : 0,
    u = (is_https ? "https": "http") + analyHost;
window.piwikAsyncInit = function() {
    var a = Piwik.getAsyncTracker(),
        b = getUserIdFromCookie();
    b && (_paq.push(["setTrackerUrl", u + "index.php"]), a.setCustomRequestProcessing(function(a) {
        var c = a + "&userID=" + b;
        return c
    }))
},
    function() {
        _paq.push(["setTrackerUrl", u + "index.php?userID=" + getUserIdFromCookie()]),
        ...
        b.type = "text/javascript",
            b.defer = !0,
            b.async = !0,
            b.src = is_https ? "https://analysis.koudai.com/piwik.js": "http://static.koudai.com/analysis/piwik.js",
            c.parentNode.insertBefore(b, c)
    } (),
    window.DECORATESHOPID = "1065304";